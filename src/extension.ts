...
function escapeGdriveQueryParam(param: string) {
    if (!param) return "";
...
export function activate(context: vscode.ExtensionContext) {
    ...
    let aiHistorySyncTimeout: NodeJS.Timeout | undefined;
...
async function toggleClipboardSync(context: vscode.ExtensionContext) {
...
async function setupCloudClipboard(context: vscode.ExtensionContext) {
    if (clipboardInterval) {
        clearInterval(clipboardInterval);
    }
        ...
        if (local && local.hash !== lastLocalHash) {
            ...
            if (syncFile && syncFile.id) {
                await drive.files.update({
                    fileId: syncFile.id,
                    requestBody: { appProperties: { machineId: machineId, hash: local.hash } },
        ...
        if (syncFile) {
            ...
            if (remoteMachineId && remoteMachineId !== machineId && remoteHash !== lastLocalHash) {
                ...
                if (content && content.type && content.data) {
                    ...
                    vscode.window.setStatusBarMessage(`📋 Буфер обновлен из облака (${content.type === 'image' ? 'Картинка' : 'Текст'})`, 3000);
                }
            }
        }
    } catch (error) {
...
async function setLocalClipboard(type: string, data: string) {
    if (type === 'text') {
        ...
        try {
            ...
            const workspaceRoot = getWorkspaceRoot();
            await runCommand(psCommand, workspaceRoot || '');
...
async function setupBranchMonitoring(context: vscode.ExtensionContext) {
    const workspaceRoot = getWorkspaceRoot();
    ...
    if (gitExtension) {
        ...
        api.onDidOpenRepository((repo: any) => subscribeToRepo(repo, context));
        api.repositories.forEach((repo: any) => subscribeToRepo(repo, context));
    } else {
...
function subscribeToRepo(repo: any, context: vscode.ExtensionContext) {
...
async function checkForBranchChanges(context: vscode.ExtensionContext) {
    ...
    try {
        ...
        for (const branch of Array.from(lastKnownBranches) as string[]) {
...
async function getLocalBranches(workspaceRoot: string) {
    ...
    vscode.window.withProgress({
        ...
        try {
            ...
            for (const file of files) {
                ...
                if (file.id && file.name && file.name.startsWith(`${sanitizedName}--`)) {
...
async function deleteUntrackedFile(context: vscode.ExtensionContext) {
    const workspaceRoot = getWorkspaceRoot();
    if (!workspaceRoot) return;
        ...
        for (const selectedFile of selectedFiles) {
            try {
                ...
                if (!remoteFile) {
                    ...
                    await moveFileToDeleted(drive, remoteFile, deletedFolderId);
                }
...
async function clearTombstones(context: vscode.ExtensionContext) {
    const workspaceRoot = getWorkspaceRoot();
    if (!workspaceRoot) return;
    ...
    try {
        ...
        const decisions: any = getConflictDecisions(context);
        let changed = false;
...
function getConflictDecisions(context: vscode.ExtensionContext): Record<string, any> {
    return context.workspaceState.get<Record<string, any>>(CONFLICT_DECISIONS_KEY, {});
}

async function setConflictDecision(context: vscode.ExtensionContext, decision: any) {
    const decisions = getConflictDecisions(context);
    decisions[decision.key] = decision.data;
    await context.workspaceState.update(CONFLICT_DECISIONS_KEY, decisions);
}

async function clearConflictDecision(context: vscode.ExtensionContext, key: string) {
    const decisions = getConflictDecisions(context);
    delete decisions[key];
    await context.workspaceState.update(CONFLICT_DECISIONS_KEY, decisions);
}

async function findOrCreateDeletedFolder(drive: drive_v3.Drive, untrackedFolderId: string, context: vscode.ExtensionContext) {
    return await ensureSingleFolder(drive, untrackedFolderId, '.deleted', context);
}

async function moveFileToDeleted(drive: drive_v3.Drive, fileToMove: any, deletedFolderId: string) {
    ...
    const originalParentId = (parents && parents.length > 0) ? parents[0] : 'root';
...
async function getAllTombstones(drive: drive_v3.Drive, deletedFolderId: string) {
...
async function syncUntrackedFiles(context: vscode.ExtensionContext, silent: boolean = false) {
    const workspaceRoot = getWorkspaceRoot();
    if (!workspaceRoot) return;
...
async function uploadUntrackedFiles(context: vscode.ExtensionContext, silent: boolean = false) {
    const workspaceRoot = getWorkspaceRoot();
    if (!workspaceRoot) return;
        ...
        for (const relativePath of filesToUpload) {
            ...
            try {
                ...
                if (remoteFile) {
                    if (localMd5 !== remoteFile.md5Checksum) {
                        const remoteMachineId = (remoteFile.appProperties && remoteFile.appProperties.machineId) ? remoteFile.appProperties.machineId : null;
                        if (remoteMachineId === machineId && remoteFile.id) {
                        ...
                        if (choice.action === 'skip') {
                            ...
                            await clearConflictDecision(context, decisionKey);
                            if (choice.action === 'upload' && remoteFile.id) {
                                await updateFile(drive, remoteFile.id, absolutePath, machineId);
                                if (!silent) vscode.window.showInformationMessage(`Выгружен: ${relativePath}`);
                            } else if (choice.action === 'download' && remoteFile.id) {
...
async function initialUpload(context: vscode.ExtensionContext) {
    const workspaceRoot = getWorkspaceRoot();
    if (!workspaceRoot) return;
...
async function getRemoteRefs(drive: drive_v3.Drive, bundleFolderId: string) {
...
async function updateRemoteRefs(drive: drive_v3.Drive, bundleFolderId: string, refs: any) {
    const q = `name='${REFS_FILE_NAME}' and '${bundleFolderId}' in parents and trashed=false`;
    const res = await drive.files.list({ q, fields: 'files(id, name)' });
    const files = res.data.files || [];
...
async function sync(context: vscode.ExtensionContext, silent: boolean = false) {
...
async function checkRemoteBranchTombstones(context: vscode.ExtensionContext, drive: drive_v3.Drive, silent: boolean = false) {
    ...
    try {
        ...
        for (const tombstone of tombstones) {
            ...
            if (localBranches.includes(branchName) && tombstone.id && !(processedTombstones as any)[tombstone.id]) {
                const choice = await vscode.window.showWarningMessage(
                ...
                if (tombstone.id) {
                    (processedTombstones as any)[tombstone.id] = true;
                }
            }
        }
        ...
        console.error('Error checking remote branch tombstones:', (error as any).message);
    }
}
    ...
    try {
        for (const [branchName, bundles] of remoteBundlesByBranch.entries()) {
            if (localBranches.has(branchName)) {
                ...
                const newBundles = bundles.filter((bundle: any) => {
...
async function installGitHooks(_context: vscode.ExtensionContext) {
    const workspaceRoot = getWorkspaceRoot();
    if (!workspaceRoot) return;
...
async function cloneFromGoogleDrive(context: vscode.ExtensionContext) {
    const workspaceRoot = getWorkspaceRoot();
    if (!workspaceRoot) return;
    ...
    try {
        ...
        const resProjects = await drive.files.list({
        ...
        const resRemote = await drive.files.list({
            q: `'${bundleFolderId}' in parents and trashed=false and fileExtension='bundle'`,
            fields: 'files(id, name)',
            orderBy: 'createdTime desc'
        });
        const remoteBundles = resRemote.data.files || [];
        ...
        const selectedBundle = await vscode.window.showQuickPick(
            (remoteBundles as any[]).map(b => ({ label: b.name!, description: `(ID: ${b.id})`, id: b.id! })),
        ...
        await downloadFile(drive, (selectedBundle as any).id, tempBundlePath);
...
async function manageSyncHash(context: vscode.ExtensionContext) {
    const workspaceRoot = getWorkspaceRoot();
    if (!workspaceRoot) return;
    ...
    const lastPushedHash = context.workspaceState.get<string>(hashKey);
    ...
    try {
        ...
        if (bundleFolderId) {
            ...
            if (newHash === undefined) {
                delete (remoteRefs as any)[currentBranch];
            } else {
                (remoteRefs as any)[currentBranch] = newHash;
            }
            await updateRemoteRefs(drive, bundleFolderId, remoteRefs);
        }
    } catch (e) {
...
async function selectCommitHash(workspaceRoot: string) {
    try {
...
async function trackCurrentConversation(context: vscode.ExtensionContext) {
    const workspaceRoot = getWorkspaceRoot();
    if (!workspaceRoot) return;
...
async function configureAIHistorySync(context: vscode.ExtensionContext) {
    const choice = await vscode.window.showQuickPick([
...
async function syncAIHistory(context: vscode.ExtensionContext, silent: boolean = false) {
    const workspaceRoot = getWorkspaceRoot();
    if (!workspaceRoot) return;
    ...
    try {
        ...
        if (!manifestFileId) {
            console.error('Cannot sync: manifest file ID is missing.');
            return;
        }

        // 1. Загружаем манифест с диска (чтобы узнать о чатах с других машин)
        const { data: manifestContent }: any = await drive.files.get({ fileId: manifestFileId, alt: 'media' });
        ...
        for (const convId of allRelevantIds) {
            ...
            if (fsSync.existsSync(localPath)) {
                ...
                await downloadFolder(drive, remoteConvFolderId, localPath);
            }
        ...
        if (manifestFileId && mergedIds.length > (remoteManifest.ids || []).length) {
            await drive.files.update({
...
async function setupGoogleCredentials(context: vscode.ExtensionContext) {
...
async function authenticateWithGoogle(context: vscode.ExtensionContext) {
...
async function getAuthenticatedClient(context: vscode.ExtensionContext) {
    const credentialsStr = await context.secrets.get(GOOGLE_DRIVE_CREDENTIALS_KEY);
...
async function findOrCreateProjectFolders(drive: drive_v3.Drive, workspaceRoot: string, context: vscode.ExtensionContext) {
    const projectFolderId = await findOrCreateBaseProjectFolder(drive, workspaceRoot, context);
    return await ensureSingleFolder(drive, projectFolderId, 'bundles', context);
}

async function findOrCreateUntrackedFilesFolder(drive: drive_v3.Drive, workspaceRoot: string, context: vscode.ExtensionContext) {
    const projectFolderId = await findOrCreateBaseProjectFolder(drive, workspaceRoot, context);
    return await ensureSingleFolder(drive, projectFolderId, 'untracked', context);
}

async function findOrCreateTombstonesFolder(drive: drive_v3.Drive, workspaceRoot: string, context: vscode.ExtensionContext) {
    const projectFolderId = await findOrCreateBaseProjectFolder(drive, workspaceRoot, context);
    return await ensureSingleFolder(drive, projectFolderId, 'tombstones', context);
}

async function findOrCreateBaseProjectFolder(drive: drive_v3.Drive, workspaceRoot: string, context?: vscode.ExtensionContext) {
...
async function findOrCreateSubFolder(drive: drive_v3.Drive, parentId: string | null, folderName: string, context?: vscode.ExtensionContext) {
    return await ensureSingleFolder(drive, parentId, folderName, context);
}

// --- УПРАВЛЕНИЕ ПАПКАМИ И СЛИЯНИЕ ---

async function ensureSingleFolder(drive: drive_v3.Drive, parentId: string | null | undefined, folderName: string, context?: vscode.ExtensionContext) {
...
async function reconcileFolderDuplicates(drive: drive_v3.Drive, folders: any[], folderName: string, parentId: string | null | undefined, context: vscode.ExtensionContext) {
    ...
    if (!backupBaseId) {
        throw new Error('Could not find or create backups folder');
    }
    ...
    for (const dup of duplicates) {
        const primaryId = (primaryFolder as any).id as string;
        const dupId = (dup as any).id as string;
        if (primaryId && dupId && backupSessionId) {
            await mergeFoldersRecursive(drive, primaryId, dupId, backupSessionId);
        }
        // Перемещаем теперь уже пустой дубликат в бэкап
        if (dup.id && backupSessionId) {
            await drive.files.update({
                fileId: dup.id,
...
async function mergeFoldersRecursive(drive: drive_v3.Drive, targetFolderId: string, sourceFolderId: string, backupFolderId: string) {
...
async function cleanupOldBackups(drive: drive_v3.Drive, workspaceRoot: string) {
    try {
...
async function findOrCreateBackupsFolder(drive: drive_v3.Drive, workspaceRoot: string, context?: vscode.ExtensionContext) {
    const projectFolderId = await findOrCreateBaseProjectFolder(drive, workspaceRoot, context);
    ...
    const { data } = await drive.files.create({
        requestBody: { name: BACKUPS_DIR_NAME, mimeType: 'application/vnd.google-apps.folder', parents: [projectFolderId] },
        fields: 'id'
    });
    return data.id;
}
...
async function getCloudConfig(drive: drive_v3.Drive, projectFolderId: string) {
...
async function updateCloudConfig(drive: drive_v3.Drive, projectFolderId: string, config: any) {
    const q = `name='${CLOUD_CONFIG_FILE_NAME}' and '${projectFolderId}' in parents and trashed=false`;
...
async function getEffectiveConfig(drive: drive_v3.Drive, projectFolderId: string) {
...
async function uploadBundleFile(drive: drive_v3.Drive, filePath: string, parentFolderId: string) {
...
async function getAllRemoteFiles(drive: drive_v3.Drive, folderId: string, recursive: boolean = false, prefix: string = '') {
    let files: any[] = [];
        ...
        fileStream.pipe(dest)
            ...
            .on('error', (error: any) => {
                ...
                if (error.code === 'EBUSY' || error.code === 'EPERM') {
                    ...
                    resolve(undefined); // Resolve to not stop the entire sync process.
                } else {
...
async function findRemoteFile(drive: drive_v3.Drive, folderId: string, fileName: string) {
...
async function uploadFile(drive: drive_v3.Drive, folderId: string, filePath: string, relativePath: string, machineId: string) {
...
async function updateFile(drive: drive_v3.Drive, fileId: string, filePath: string, machineId: string) {
    const media = { mimeType: 'application/octet-stream', body: fsSync.createReadStream(filePath) };
    await drive.files.update({
        fileId: fileId,
...
function sanitizeBranchNameForDrive(branchName: string) {
...
function restoreBranchNameFromDrive(driveBranchName: string) {
...
async function getUntrackedAndIgnoredFiles(workspaceRoot: string) {
    try {
        ...
        for (const dir of dirs) {
            ...
            try {
                ...
                if (stat.isDirectory()) {
                    ...
                    subFiles.forEach((f: string) => allFiles.add(f));
                } else {
